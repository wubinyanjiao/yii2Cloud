...
class Employee extends BaseEmployee
{
    ...
    public function useradd($data,$emp_number,$workStation){

        $arr = User::find()->where(['user_name'=>$data['user_name']])->one();
        if($arr != ''){
            return 2;
        }
        ...
        if($info == ''){
            ...
            if($workStation != 0){
                $employee->work_station = $workStation;
            }
    ...
    * **/
    ...
    public function group($id){
        $query = Employee::find()->select(['emp_number','emp_firstname'])->where(['work_station'=>$id])->andWhere(['termination_id' => null])->all();
        return $query;
    }
    ...
    public function getEmpByWorkStation($work_station){
        $query = Employee::find()
            ...
            if($work_station){
                $query->where(['work_station'=>$work_station]);
            }
    ...
    public function getEmpByNumNber($empNumber){
        if(is_array($empNumber)){
            $query = Employee::find()->where(['in','emp_number',$empNumber])->all();
        }else{
    ...
    public function getEmpNumberByGenderSubunit($gender = 0,$workStation = null){
        $query = Employee::find();
        ...
        if(!empty($gender)&&$gender>0){
            $query->andWhere(['emp_gender'=>$gender]);;
        }
     ...
     *
     */
    public function getCountJiabanByEmpNmber($stat_date,$end_date,$empNmber){
        try {
            
            $q = Overtime::find();
            $q->select('SUM(ohrm_overtime_list.hour_differ) as sum')   ;
            $q->where('ohrm_overtime_list.emp_number = :empNmber',[':empNmber'=>$empNmber]);
            $q->andWhere('ohrm_overtime_list.status = 2');
            $q->andWhere('ohrm_overtime_list.is_holiday != 1');
            $q->andWhere("ohrm_overtime_list.current_day <= :end_date", [':end_date'=>$end_date]);
            $q->andWhere("ohrm_overtime_list.current_day >= :stat_date",[':stat_date'=>$stat_date]); 
            //echo $q->createCommand()->getRawSql();die;
            $result = $q->asArray()->one();
     ...
     *
     */
    public function getCountYebanByEmpNumber($stat_date,$end_date,$empNmber){
        try {
            $q = ShiftResult::find();

 
                $q->joinWith('schedule');
                $q->joinWith('shiftType');         
                      
                $q->where('ohrm_work_shift_result.emp_number = :empNmber',[':empNmber'=>$empNmber]);
                $q->andWhere('ohrm_work_shift_type.is_night_shift = 1') ;
                $q->andWhere('ohrm_work_schedule.is_show = 1');
                $q->andWhere('ohrm_work_schedule.is_confirm = 1');
                $q->andWhere("ohrm_work_shift_result.shift_date <= :end_date", [':end_date'=>$end_date]);  
                $q->andWhere(" ohrm_work_shift_result.shift_date >= :stat_date",[':stat_date'=>$stat_date]);                         
            $result = $q->count();
            return $result;
     ...
     /**
     * pan 1/10
     * 根据用户名和班次名称获取员工的一段时间上班数量
     *
     */
    public function getCountGangweiBanci($stat_date,$end_date,$empNmber,$banci_name){
        try {
            $q = ShiftResult::find();
                ...
                $q->where('ohrm_work_shift_result.emp_number = :empNmber',[':empNmber'=>$empNmber]);
                $q->andWhere('ohrm_work_shift_type.name = :name',[':name'=>$banci_name]) ;
                $q->andWhere('ohrm_work_schedule.is_show = 1');
                $q->andWhere('ohrm_work_schedule.is_confirm = 1');
                $q->andWhere("ohrm_work_shift_result.shift_date <= :end_date", [':end_date'=>$end_date]);  
                $q->andWhere(" ohrm_work_shift_result.shift_date >= :stat_date",[':stat_date'=>$stat_date]);    
     ...
     *
     */
    public function getCountWorkLoad($stat_date,$end_date,$empNumber){
        try {
            
            $query = (new \yii\db\Query())
                ->select('SUM(a.work_check) as sum')
                ->from('orangehrm_mysql.ohrm_work_load a')
                ->leftJoin('orangehrm_mysql.hs_hr_employee b','a.employee_id=b.emp_number')   
                ->where('a.employee_id = :empNumber',[':empNumber'=>$empNumber])
                ->andWhere('b.workload_ranking = 1')
                ->andWhere("a.work_date <= :end_date", [':end_date'=>$end_date])
                ->andWhere(" a.work_date >= :stat_date",[':stat_date'=>$stat_date]) 
                ->one();

             if($query['sum']) {
                $result = floatval($query['sum']);
             } else{
                $result = 0;
             }
