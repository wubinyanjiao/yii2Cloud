...
class EmployeeController extends \common\rest\Controller
{
    ...
    public function actionUserAdd(){
        ...
        $employee = new Employee();
        $model = $employee->useradd($data,$emp_number,$workStation);
        if($model==false){
        ...
        if($model === 2){
            $this->serializer['message'] ='用户名已经存在';
            $this->serializer['status'] =false;
            return false;
        }
        return $model;
   ...
   * **/
    ...
    /**
     * @SWG\Post(path="/employee/user-name",
     *     tags={"云平台-Employee-用户管理"},
     *     summary="添加时获取工资号",
     *     description="添加时获取工资号",
     *     produces={"application/json"},
     *     @SWG\Parameter(
     *        in = "formData",
     *        name = "Token",
    ...
    public function actionUserName(){
        $where = "user_name like '%1000%'";
        $user = User::find()->select(['user_name'])->asArray()->where($where)->all();
        $arr = max($user);
        $arr = $arr['user_name']+1;
        return $arr;
    }
