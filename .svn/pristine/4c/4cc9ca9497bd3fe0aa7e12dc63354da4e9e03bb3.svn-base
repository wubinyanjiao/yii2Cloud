...
namespace common\models\RotationRule;

use Yii;
use \common\models\rotationrule\base\RotationRule as BaseAttendanceRecord;
 ...
 */
class RotationRule extends BaseAttendanceRecord
{
     ...
     * 获取所有的临时数据
     */
    public function RotationRuleList($rotaryId)
    {
        $query = self::find();
        $query->where("rotaryid = :rotaryid",[':rotaryid'=>$rotaryId]);
        $list = $query->one();
        return $list;
    }
    /*
     * 获取所有的轮转列表
     */
    public function RotationRule(){
        $query = RotationRule::find()->all();
        return $query;
